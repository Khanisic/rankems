...
function GamePage() {
    ...
    return (
        <div className="bg-bg min-h-screen pb-10 px-8 md:p-16 flex flex-col items-center justify-center">
            <head>
                <title>{game.title} - Vote & Rank Now | Rankems</title>
                <meta name="description" content={`Join the interactive ranking for "${game.title}"! Drag and drop to rank items across ${game.categories.length} categories. ${game.votesCount} people have already voted in this ${game.votingMode} ranking game.`} />
                <meta name="keywords" content={`${game.title}, ranking game, vote now, interactive ranking, ${game.categories.join(', ')}, game ${game.id}, rankems voting`} />
                <meta name="author" content="Abdul Moid Khan" />
                <meta name="robots" content="index, follow" />
                <meta name="viewport" content="width=device-width, initial-scale=1.0" />
                <link rel="canonical" href={`https://rankems.xyz/game/${game.id}`} />
                
                {/* Open Graph / Facebook */}
                <meta property="og:type" content="website" />
                <meta property="og:url" content={`https://rankems.xyz/game/${game.id}`} />
                <meta property="og:title" content={`${game.title} - Vote & Rank Now`} />
                <meta property="og:description" content={`Join the interactive ranking for "${game.title}"! Drag and drop to rank items across ${game.categories.length} categories. ${game.votesCount} people have already voted.`} />
                <meta property="og:image" content="https://rankems.xyz/og-game.jpg" />
                ...
                <meta property="twitter:url" content={`https://rankems.xyz/game/${game.id}`} />
                <meta property="twitter:title" content={`${game.title} - Vote & Rank Now`} />
                <meta property="twitter:description" content={`Join the interactive ranking for "${game.title}"! ${game.votesCount} people have already voted. Add your rankings now!`} />
                <meta property="twitter:image" content="https://rankems.xyz/twitter-game.jpg" />
                ...
                <script type="application/ld+json">
                  {JSON.stringify({
                    ...
                    "@type": "Game",
                    "name": game.title,
                    "description": `Interactive ranking game: ${game.title}. Rank items across ${game.categories.length} categories.`,
                    "url": `https://rankems.xyz/game/${game.id}`,
                    "gameLocation": `https://rankems.xyz/game/${game.id}`,
                    "numberOfPlayers": game.votesCount,
                    ...
                    "gameItem": game.friends.map(friend => ({
                      "@type": "Thing",
                      "name": friend
                    })),
                    "about": game.categories.map(category => ({
                      "@type": "Thing",
                      "name": category
                    })),
                    "potentialAction": {
                      "@type": "PlayAction",
                      "target": `https://rankems.xyz/game/${game.id}`,
                      "expectsAcceptanceOf": {
                        "@type": "Offer",
                        "price": "0",
                        "priceCurrency": "USD"
